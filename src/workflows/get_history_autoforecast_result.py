...
class GetHistoryAutoForecastResultWorkflow(BaseWorkflow):
    ...
    2. 解析会话参数 - 提取预报对象和降雨大概起止时间
    3. 获取历史面雨量过程 - 调用forecast_rain_ecmwf_avg工具
    4. 获取具体降雨起止时间 - 分析降雨过程数据
    5. 获取历史洪水自动预报方案ID - 调用get_history_autoforcast_list工具
    6. 获取历史洪水自动预报结果 - 调用get_tjdata_result工具
    ...
    async def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 步骤5: 获取历史洪水自动预报方案ID
            logger.info("执行步骤5: 获取历史洪水自动预报方案ID")
            start_time = time.time()
            ...
            logger.info("执行步骤6: 获取历史洪水自动预报结果")
            start_time = time.time()
            ...
            if forecast_result.data and isinstance(forecast_result.data, dict):
                logger.info(f"步骤6返回数据keys: {list(forecast_result.data.keys())}")
            elif not forecast_result.data:
                logger.warning("步骤6返回数据为空!")
    ...
    async def execute_step(self, state: Dict[str, Any], step_index: int) -> Dict[str, Any]:
        ...
        try:
            if step_id == 1:
                ...
                return await self._step_2_parse_params(ctx, parsed_params)
            elif step_id == 3:
                return await self._step_3_get_history_rain(ctx, parsed_params, registry)
            elif step_id == 4:
                return await self._step_4_analyze_rain_time(ctx)
            elif step_id == 5:
                return await self._step_5_get_plan_id(ctx, registry)
            elif step_id == 6:
                return await self._step_6_get_result(ctx, registry)
            elif step_id == 7:
                return await self._step_7_extract_result(ctx, parsed_params)
            else:
    ...
    async def _step_2_parse_params(self, ctx: Dict, parsed_params: Dict) -> Dict[str, Any]:
        """步骤2: 解析会话参数"""
    ...
    async def _step_3_get_history_rain(self, ctx: Dict, parsed_params: Dict, registry) -> Dict[str, Any]:
        """步骤3: 获取历史面雨量过程"""
    ...
    async def _step_4_analyze_rain_time(self, ctx: Dict) -> Dict[str, Any]:
        """步骤4: 获取具体降雨起止时间"""
    ...
    async def _step_5_get_plan_id(self, ctx: Dict, registry) -> Dict[str, Any]:
        """步骤5: 获取历史洪水自动预报方案ID"""
    ...
    async def _step_7_extract_result(self, ctx: Dict, parsed_params: Dict) -> Dict[str, Any]:
        """步骤7: 结果信息提取整理"""
