...
async function init() {
    ...
    try {
        ...
        if (floodData) {
            await processAllData(floodData, rainData, currentStatus, stationInfoList);
        } else {
...
async function fetchObservedProcess(forecastStartTime) {
    ...
    try {
        ...
        const response = await fetch(`${API_URLS.riverProcess}?${params}`, { headers, cache: 'no-cache' });
        if (!response.ok) return [];
        const result = await response.json();
        return extractProcessRows(result)
            .map(row => normalizeObservedReachRow(row))
            .filter(row => row.time && Number.isFinite(row.timestamp) && row.timestamp <= startTimestamp)
            .sort((a, b) => a.timestamp - b.timestamp);
    } catch (error) {
        console.warn("获取河道站点实测过程失败:", error);
        return [];
    }
}
...
async function processAllData(floodRaw, rainData, currentStatus, stationInfoList) {
    ...
    try {
        const observedData = await fetchObservedProcess(getForecastStartTime(stationData));
        renderProcessChart(stationData, rainData, currentStationInfo, observedData);
    } catch (e) {
 ...
 */
function renderProcessChart(data, rainData, stationInfo, observedData = []) {
    ...
    const timeKeys = objectKeys(zMap).length > 0 ? objectKeys(zMap) : objectKeys(qMap);
    ...
    const levelSeries = splitForecastSeries({ name: '水位', data: waterLevelData, observedData: observedSeries?.level, color: '#00d4ff', fill: true });
    const flowSeries = splitForecastSeries({ name: '流量', data: flowData, observedData: observedSeries?.flow, color: '#4ade80' });
    ...
    if (rainData && Array.isArray(rainData)) {
        processedRainData = rainData
            .map(d => [toTimestamp(d.time), Number(d.value)])
    ...
    // 水位轴范围需要考虑警戒水位
    const waterLevelRange = calcAxisRange(levelSeries.map(s => s.data), 5, 2, [warningZ]);
    ...
    option.series = [
        option.series[0],
        ...levelSeries.map((s, index) => buildLineSeries(s, 2, peakWaterLevel, ' m', index === 0 ? waterLevelMarkLine : undefined)),
        ...flowSeries.map(s => buildLineSeries(s, 1, peakFlow, ' m³/s')),
...
function buildLineSeries(series, yAxisIndex, peak, unit, markLine) {
    return {
        name: series.name,
        type: 'line',
        xAxisIndex: 1,
...
function getForecastStartTime(data) {
    const timeKeys = objectKeys(
        data?.Level_Dic || data?.Z_Dic || data?.Discharge_Dic || data?.Q_Dic || data?.InQ_Dic || data?.OutQ_Dic
    );
